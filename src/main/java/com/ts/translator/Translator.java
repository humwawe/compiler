...
public class Translator {
    ...
    public TAProgram translate(ASTNode astNode) throws ParseException {
        ...
        if (symbolTable.exists(main)) {
            ...
            program.add(new TAInstruction(TAInstructionType.CALL, null, null, symbolTable.cloneFromSymbolTree(main, 0), null));
            program.add(new TAInstruction(TAInstructionType.SP, null, null, symbolTable.localSize(), null));
        }
        return program;
    }
    ...
    public Symbol translateExpr(
        ...
        if (node.isValueType()) {
            ...
            TAInstruction instruction = new TAInstruction(TAInstructionType.ASSIGN, (Symbol) (node.getProp("addr")), node.getLexeme().getValue(), node.getChild(0).getProp("addr"), node.getChild(1).getProp("addr"));
    ...
    private Symbol translateCallExpr(TAProgram program, ASTNode node, SymbolTable symbolTable) throws ParseException {
        ...
        List<TAInstruction> list = new ArrayList<TAInstruction>();
        ...
        Symbol funcAddr;
        funcAddr = symbolTable.cloneFromSymbolTree(factor.getLexeme(), 0);
        if (funcAddr == null) {
